...
export default class VehicleCategoryController {
  constructor(private vehicleCategoryRepository: VehicleCategoryRepository) {}
  async createVehicleCategory(req: Request, res: Response, next: NextFunction) {
    try {
      const { id } = req.params;
      const { name } = <VehicleCategory>req.body;
  ...
  async findAllVehicleCategories(
    ...
    try {
      const vehicleCategories =
        await this.vehicleCategoryRepository.findAllVehicleCategories();
  ...
  async updateVehicleCategoryById(
    ...
    try {
      ...
      const vehicleCategory =
        await this.vehicleCategoryRepository.updateVehicleCategoryById(
          Number(id),
          { name } as VehicleCategory
        );
      return res.status(200).send(vehicleCategory);
    } catch (error) {
      console.log(error);
  ...
  async deleteVehicleCategoryById(
    ...
    try {
      ...
      const vehicleCategory =
        await this.vehicleCategoryRepository.deleteVehicleCategoryById(
          Number(id)
        );
      return res.status(200).send(vehicleCategory);
    } catch (error) {
