...
export default class VehiclePhotoController {
  ...
  async findVehiclePhotoById(req: Request, res: Response, next: NextFunction) {
    try {
      ...
      const vehiclePhoto =
        await this.vehiclePhotoRepository.findVehiclePhotoById(Number(id));
      return res.status(200).send(vehiclePhoto);
    } catch (error) {
      console.log(error);
      next(error);
  ...
  async updateVehiclePhotoById(
    ...
    try {
      ...
      const vehiclePhoto =
        await this.vehiclePhotoRepository.updateVehiclePhotoById(Number(id), {
          url,
          vehicle,
        } as VehiclePhoto);
      return res.status(200).send(vehiclePhoto);
  ...
  async deleteVehiclePhotoById(
    ...
    try {
      ...
      const vehiclePhoto =
        await this.vehiclePhotoRepository.deleteVehiclePhotoById(Number(id));
      return res.status(200).send(vehiclePhoto);
    } catch (error) {
      console.log(error);
      next(error);
