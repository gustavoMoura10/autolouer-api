...
export default class VehicleFuelController {
  constructor(private vehicleFuelRepository: VehicleFuelRepository) {}
  async createVehicleFuel(req: Request, res: Response, next: NextFunction) {
    try {
      const { id } = req.params;
      const { name } = <VehicleFuel>req.body;
  ...
  async findVehicleFuelById(req: Request, res: Response, next: NextFunction) {
    try {
      ...
      const vehicleFuel = await this.vehicleFuelRepository.findVehicleFuelById(
        Number(id)
      );
      return res.status(200).send(vehicleFuel);
    } catch (error) {
  ...
  async findAllVehicleFuels(req: Request, res: Response, next: NextFunction) {
    try {
      const vehicleFuels =
        await this.vehicleFuelRepository.findAllVehicleFuels();
  ...
  async updateVehicleFuelById(req: Request, res: Response, next: NextFunction) {
    try {
      ...
      const vehicleFuel =
        await this.vehicleFuelRepository.updateVehicleFuelById(Number(id), {
          name,
        } as VehicleFuel);
      return res.status(200).send(vehicleFuel);
    } catch (error) {
  ...
  async deleteVehicleFuelById(req: Request, res: Response, next: NextFunction) {
    try {
      ...
      const vehicleFuel =
        await this.vehicleFuelRepository.deleteVehicleFuelById(Number(id));
      return res.status(200).send(vehicleFuel);
    } catch (error) {
      console.log(error);
      next(error);
