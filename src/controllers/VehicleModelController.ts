...
export default class VehicleModelController {
    ...
    async createVehicleModel(req: Request, res: Response, next: NextFunction) {
        try {
            const { id } = req.params;
            const { brand, name, vehicleTypes, bio, photo } = <VehicleModel>(
                req.body
            );
            ...
            for (let vehicleType of vehicleTypes) {
                ...
                if (vehicleTypeEntity)
                    vehicleTypeEntities.push(vehicleTypeEntity);
            }
            ...
            const vehicleModel =
                await this.vehicleModelRepository.createVehicleModel({
                    brand: brandEntity,
                    name,
    ...
    async findVehicleModelById(
        ...
        try {
            ...
            const vehicleModel =
                await this.vehicleModelRepository.findVehicleModelById(
                    Number(id)
                );
            return res.status(200).send(vehicleModel);
        } catch (error) {
    ...
    async deleteVehicleModelById(
        ...
        try {
            ...
            const vehicleModel =
                await this.vehicleModelRepository.deleteVehicleModelById(
                    Number(id)
                );
            return res.status(200).send(vehicleModel);
        } catch (error) {
